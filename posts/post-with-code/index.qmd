...
with pyodbc.connect(conn_str) as conn:
    with conn.cursor() as cursor:
        ...
        insert_query = """
            INSERT INTO some_table (field1, field2, field3)  # the actual names in the database
            VALUES (?, ?, ?)
        """
        cursor.execute(insert_query, (field1, field2, field3))  # the variables in Python
        conn.commit()
...
A vibe-coding developer might stop here, assured by the LLM that this is good implementation because:
    ...
    - Placeholders (the '?') are being used to protect against SQL injection.
